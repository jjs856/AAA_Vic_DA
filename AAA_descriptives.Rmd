...
```{r Reading in Data}
#getwd()
#setwd("~INTERACT/veritas")

#add wave one data
victoria_new <- read_csv("~/INTERACT/veritas/health_1vic_main_bfd440e.csv") #victoria health survey
victoria_eligibility <- read_csv("~/INTERACT/veritas/eligibility_1vic_main_bfd440e.csv") #victoria eligibility survey
victoria_eligibility[, "interact_id"] <- apply(victoria_eligibility[, "interact_id"], 1, function(x) as.integer(x))
victoria_new[, "interact_id"] <- apply(victoria_new[, "interact_id"], 1, function(x) as.integer(x)) #getting both health and eligibility survey to have the same INTERACT ID (one file started with 0101 and the other started with 101)

#Add Wave 2 Data here
victoria_new_two <- read_csv("~/INTERACT/veritas/health_2vic_main_bfd440e.csv") # w2
victoria_eligibility_two <- read_csv("~/INTERACT/veritas/eligibility_2vic_main_bfd440e.csv")# w2
victoria_eligibility_two[, "interact_id"] <- apply(victoria_eligibility_two[, "interact_id"], 1, function(x) as.integer(x))
victoria_new_two[, "interact_id"] <- apply(victoria_new_two[, "interact_id"], 1, function(x) as.integer(x))

# May not line up (differences in dfs??)

# IM STUCK. Error: Argument 5 must have names
victoria_merged <- bind_rows(victoria_eligibility, victoria_new, victoria_new_two, victoria_eligibility_two, by = "interact_id") #merge eligibility and health survey together
...
## Reading in SD data with ID's
```{r}
SD_vic_one <- read_csv("~/INTERACT/SenseDoc/victoria_01_top_1min.csv") # read in SD w1 participants
SD_vic_two <- read_csv("~/INTERACT/SenseDoc/victoria_02_top_1min.csv") # read in SD w2 participants 

# This will not work wither
SD_vic_all <- full_join(SD_vic_one, SD_vic_two, by = "interact_id")
...
victoria_new_ID_gps <- mutate(victoria_new_ID_gps, "age_calculated" = 2019 - year)
```
